...
import { ref, onValue, push, set, update, serverTimestamp, remove } from 'firebase/database';
import { db } from '../firebase'; 
import userService from '../services/userService';
import { FaceSmileIcon, PuzzlePieceIcon, UserPlusIcon, NoSymbolIcon, TrashIcon, PaperAirplaneIcon } from '@heroicons/react/24/solid';
...
const WouldYouRatherMessage = ({ msg, userID, targetUserID, onVote, messageKey }) => {
    ...
    return (
        <div className="shrink-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-5 rounded-[2rem] shadow-xl my-4 w-full max-w-sm mx-auto text-center border border-white/50 dark:border-gray-700/50 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-monster-turquoise to-monster-purple"></div>
            <h3 className="font-extrabold text-xl mb-4 text-gray-800 dark:text-white mt-2">Would You Rather...</h3>
            <img src={msg.imageUrl || 'https://placehold.co/400x200'} alt="game visual" className="rounded-2xl mb-5 w-full h-40 object-cover shadow-inner" />
            
            <div className="flex flex-col gap-3">
                <button 
                    onClick={() => handleVoteClick(1)} 
                    disabled={myVote || hasBothVoted} 
                    className={`w-full p-3.5 rounded-2xl text-sm font-extrabold transition-all shadow-sm ${myVote === 1 ? 'bg-monster-turquoise text-white shadow-lg transform scale-[1.02]' : 'bg-gray-50 dark:bg-gray-700/50 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600'} ${myVote && myVote !== 1 ? 'opacity-40 grayscale' : ''} ${hasBothVoted && opponentVote === 1 ? 'ring-2 ring-monster-purple ring-offset-2 dark:ring-offset-gray-800' : ''}`}
                >
                    {getOptionLabel(1)}
                </button>
                <button 
                    ...
                    disabled={myVote || hasBothVoted} 
                    className={`w-full p-3.5 rounded-2xl text-sm font-extrabold transition-all shadow-sm ${myVote === 2 ? 'bg-monster-purple text-white shadow-lg transform scale-[1.02]' : 'bg-gray-50 dark:bg-gray-700/50 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600'} ${myVote && myVote !== 2 ? 'opacity-40 grayscale' : ''} ${hasBothVoted && opponentVote === 2 ? 'ring-2 ring-monster-turquoise ring-offset-2 dark:ring-offset-gray-800' : ''}`}
                >
                    {getOptionLabel(2)}
                </button>
            </div>

            {!myVote && !opponentVote && <p className="text-xs font-bold text-gray-400 dark:text-gray-500 mt-5">Waiting for votes...</p>}
            {myVote && !opponentVote && <p className="text-xs font-bold text-monster-purple mt-5 animate-pulse">Vote sent! Waiting for them...</p>}
            {!myVote && opponentVote && <p className="text-xs font-bold text-monster-turquoise mt-5 animate-bounce">They voted! Your turn...</p>}
            {hasBothVoted && <p className="text-sm font-extrabold text-green-500 mt-5">Both have voted! Results are in.</p>}
        </div>
    );
};
...
const Chat = ({ userID, targetUser, currentUser }) => {
    ...
    // --- OYUN BAŞLATMA FONKSİYONU (DÜZELTİLDİ VE GÜVENLİ HALE GETİRİLDİ) ---
    const handleStartGame = async () => {
        if (!canSendMessage) return;
        try {
            const gameQuestion = await userService.getRandomGame(targetUserID);
            
            // Günlük 5 oyun sınırına takıldıysa veya hata varsa
            ...
            set(push(ref(db, `conversations/${conversationId}/messages`)), { 
                ...
                optionOneText: gameQuestion.optionOneText, // Yeni DB şemasıyla eşleştirildi
                optionTwoText: gameQuestion.optionTwoText, // Yeni DB şemasıyla eşleştirildi
                imageUrl: gameQuestion.imageUrl || 'https://placehold.co/400x200', 
                senderId: userID, 
                timestamp: serverTimestamp(), 
                votes: {} 
            });
    ...
    return (
        <div className="flex flex-col h-full relative bg-[#F8FAFC] dark:bg-[#0B1120] rounded-r-[2.5rem] overflow-hidden">
...
export function ChatPage({ currentUser }) { 
    ...
    useEffect(() => {
        ...
        const fetchFriendsAndChats = async () => {
            try {
                ...
                let allChatUsers = [...friendData.map(f => ({...f, userID: f._id}))];
                const friendIds = friendData.map(f => f._id);
                    ...
                    for (let key of myConversations) {
                        ...
                        if (otherUserId && !friendIds.includes(otherUserId)) {
                            try {
                                ...
                                if (!allChatUsers.find(u => u.userID === otherUserId)) {
                                    allChatUsers.push({...otherUserProfile, userID: otherUserProfile._id});
                                }
                            } catch (e) {
                                console.error("Could not fetch profile for user:", otherUserId);
                            }
                        }
                    }
                }
                ...
                console.error("Error fetching friends or suggestions:", error);
            } finally { 
                setLoading(false); 
            }
        };
    ...
    return (
        <div className="h-[calc(100vh-70px)] w-full flex justify-center items-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 font-sans p-4 lg:p-6 transition-colors">
            <div className="w-full max-w-7xl h-full max-h-[850px] bg-white/90 dark:bg-gray-800/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl overflow-hidden flex border border-white/40 dark:border-gray-700/50">
                
                {/* SOL MENÜ (Kullanıcı Listesi) */}
                <div className="w-[35%] lg:w-[30%] bg-gray-50/50 dark:bg-gray-800/50 border-r border-gray-100 dark:border-gray-700/50 flex flex-col backdrop-blur-md z-10">
                    <div className="flex-shrink-0 h-full overflow-hidden flex flex-col">
                        <div className="flex-grow overflow-y-auto custom-scrollbar">
                            {/* NOTE: If ChatUserList isn't imported correctly, make sure it is at the top */}
                            {/* <ChatUserList users={chatUsers} onSelectUser={setSelectedUser} selectedUser={selectedUser} currentUserID={currentUser._id} /> */}
                        </div>
                ...
                {/* SAĞ MENÜ (Sohbet Ekranı veya Boş Ekran) */}
                <div className="w-[65%] lg:w-[70%] flex flex-col bg-transparent relative z-0">
                    {selectedUser ? (
                        ...
                        <div className="flex-grow flex flex-col items-center justify-center text-center p-8 relative overflow-hidden bg-[#F8FAFC] dark:bg-[#0B1120] rounded-r-[2.5rem]">
                            ...
                            <div className="relative z-10 flex flex-col items-center">
                                <div className="w-28 h-28 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center mb-8 shadow-2xl border border-white dark:border-gray-700 transform transition-transform hover:scale-110 duration-500">
                                    <svg className="h-12 w-12 text-monster-purple" fill="none" viewBox="0 0 24 24" stroke="currentColor">
