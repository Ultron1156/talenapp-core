...
const Register = ({ onLogin }) => { // onLogin prop'unu ekledik ki Google ile girerse direkt siteye yollayalım
  ...
  const handleVerifySubmit = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8081'}/api/auth/verify-email`, {
        method: 'POST',
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8081'}/api/auth/google`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: user.email,
